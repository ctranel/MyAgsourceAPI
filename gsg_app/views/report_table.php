					...
					if(is_array($fields)):
						...
						foreach($fields as $field_display => $field_name):
							...
							if(isset($arr_field_links[$field_name])){
								...
								if(substr($link, 0, 1) == '#') $value = anchor($link, $value, $rel . $title . $class);
								elseif(strpos($link, 'http') === FALSE || strpos($link, 'myagsource.com') !== FALSE) $value = anchor(site_url($link . $site_params), $value, $rel . $title . $class);
								else $value = anchor(site_url($link . '?' . $params), $value, $rel . $title . $class);
							}
