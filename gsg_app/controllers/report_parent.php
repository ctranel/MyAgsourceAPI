...
//			'benchmarks_id' => $this->arr_filter_criteria['benchmarks_id'],
	...
	protected function load_chart(&$arr_this_block, $report_count){
		...
		$this->graph['data'] = $this->{$this->primary_model}->get_graph_data($arr_fieldnames, $this->session->userdata('herd_code'), $this->max_rows, $x_axis_date_field, $this->graph['config']['xAxis']['categories']);
	}
	...
	protected function load_table(&$arr_this_block, $report_count){
		...
		$results = $this->{$this->primary_model}->search($this->session->userdata('herd_code'), $this->arr_filter_criteria, $this->arr_sort_by, $this->arr_sort_order, $this->max_rows);
