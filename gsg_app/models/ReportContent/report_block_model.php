...
/*
		if(!is_array($arr_groupings) || empty($arr_groupings)){
			$arr_groupings = $this->db
				->query("
				SELECT bf.block_id, 1 AS id, bf.header_text AS text, NULL AS parent_id, bf.list_order, hl.content_id AS header_group_id, hl.a_href, hl.a_title, hl.a_rel, hl.a_class, hc.comment
				FROM users.dbo.blocks_select_fields bf
					LEFT JOIN users.dbo.db_fields f ON bf.field_id = f.id
					LEFT JOIN users.dbo.supp_links hl ON bf.id = hl.content_id AND hl.content_type_id = 4
					LEFT JOIN users.dbo.supp_comments hc ON bf.id = hc.content_id AND hc.content_type_id = 4
