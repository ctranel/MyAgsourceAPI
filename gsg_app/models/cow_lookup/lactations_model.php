...
class Lactations_model extends CI_Model {
	...
	public function getLactationsArray($herd_code, $serial_num){
		...
		->get('vma.dbo.vma_Cow_Lookup_Lactations')
		->result_array();
	...
	public function getDamLactationsArray($herd_code, $serial_num){
		$arr_ret = $this->{$this->db_group_name}
		...
		->where('herd_code', $herd_code)
		->where('serial_num', $serial_num)
		->get('animal.dbo.view_ped_dam_lactation')
		->result_array();
	...
	public function getDamOffspringArray($herd_code, $serial_num){
		$arr_ret = $this->{$this->db_group_name}
		->select("calf_control_num, FORMAT(calving_date, 'd') calving_date, calf_name, calf_visible_id, sex_desc, twin_code, calving_ease_code, calf_sire_naab, calf_sire_name")
		->where('herd_code', $herd_code)
		->where('serial_num', $serial_num)
		->get('animal.dbo.view_ped_dam_calving')
		->result_array();
