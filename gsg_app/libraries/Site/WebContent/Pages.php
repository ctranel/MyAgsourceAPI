...
class Pages implements iWebContentRepository {
	...
	public function getByPath($path){
		$criteria = ['url_segment' => $path];
		$results = $this->datasource_pages->getByCriteria($criteria);
		if(empty($results)){
			return false;
		}
		return new Page($results[0]['id'], $results[0]['section_id'], $results[0]['name'], $results[0]['description'], $results[0]['scope_id'], $results[0]['active'], $results[0]['url_segment']);
	}
