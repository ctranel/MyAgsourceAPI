...
console.debug(s);
            if ( s.regression && !s.rendered ) {
                ...
                var extraSerie = {
                        ...
                        name: s.regressionSettings.name || s.name + ' Trend',//"Equation: %eq", 
    ...
    function _polynomial(data, order) {
        ...
        //ctranel: use the middle data chunk from data point (think 50th ptile on boxplot) to calculate regression/trend
        var position = Math.ceil(((data[0].length - 1) / 2));
        
        for (; i < k; i++) {
            for (var l = 0, len = data.length; l < len; l++) {
                if (data[l]['x']) {
                    data[l][0] = data[l]['x'];
                    data[l][position] = data[l]['y'];
                }
                if (data[l][position]) {
                    a += Math.pow(data[l][0], i) * data[l][position];
                }
            }
            lhs.push(a), a = 0;
            var c = [];
...
//console.debug(data);
        ...
        //ctranel: use the middle data chunk from data point (think 50th ptile on boxplot) to calculate regression/trend
        var position = Math.ceil(((data[0].length - 1) / 2))  
        var yval = data.map(function(pair){return pair[position]});
