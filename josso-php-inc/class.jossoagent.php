...
class jossoagent  {
  ...
  */
  ...
  /**
   * SOAP Client for identity provider.
   * @var string
   * @access private
   */
  ...
  static function getNewInstance() {
    ...
    global $josso_gatewayLoginUrl, $josso_gatewayLogoutUrl, $josso_endpoint,
    $josso_proxyhost, $josso_proxyport, $josso_proxyusername, $josso_proxypassword,
    $josso_agentBasecode, $josso_p3pHeaderValue;
  ...
  $josso_proxyhost, $josso_proxyport, $josso_proxyusername,
  $josso_proxypassword, $josso_agentBasecode, $josso_p3pHeaderValue)
  {
    // WS Config
  ...
  function isAutomaticLoginRequired() {
    // TODO : This is not the best way to avoid loops when no referrer is present,
    // the flag should expire and should not be attached to the SSO Session

    // The first time we access a partner application, we should attempt an automatic login.
    // If no referrer host is found but we did not executed auto login yet, give it a try.
  ...
  function resolveAuthenticationAssertion($assertionId) {
    // SOAP Invocation
    // echo("resolveAuthenticationAssertion ".$assertionId );

    $soapclient = $this->getIdentityProviderSoapClient();

    $resolveAuthenticationAssertionRequest =
       array('ResolveAuthenticationAssertionRequest' => array('assertionId' => $assertionId));
    
    $resolveAuthenticationAssertionResponse =
       $soapclient->call('resolveAuthenticationAssertion', $resolveAuthenticationAssertionRequest);

//     if($this->checkError($soapclient))
//     {
//       echo("resolveAuthenticationAssertion ".$resolveAuthenticationAssertionResponse."|<br/>" );
//       echo(" err ".$this->err." fault ".$this->fault."|<br/>" );
//     }
  ...
  function getIdentityMgrSoapClient() {
    // Lazy load the proper soap client
    if (!isset($this->identityMgrClient)) {
      $this->identityMgrClient =
         new nusoapclient($this->endpoint . '/josso/services/SSOIdentityManager?wsdl', true,
   ...
   * @access private
   */
  function getIdentityProviderSoapClient() {
    // Lazy load the propper soap client
    if (!isset($this->identityProviderClient))
    {
      $this->identityProviderClient =
        new nusoapclient($this->endpoint . '/josso/services/SSOIdentityProvider?wsdl', true,
          $this->proxyhost, $this->proxyport, $this->proxyusername, $this->proxypassword);
  ...
  function getSessionMgrSoapClient() {
    ...
    if (!isset($this->sessionMgrClient)) {
      ...
      $this->sessionMgrClient =
         new nusoapclient($this->endpoint . '/josso/services/SSOSessionManager?wsdl', true,
      $this->proxyhost, $this->proxyport, $this->proxyusername, $this->proxypassword);
    }
