...
<body style="font-family: Helvetica,Arial,Verdana,sans-serif">
	...
	<p><?php echo $first_name . ' ' . $last_name; ?><?php if(!empty($company)) echo ' of ' . $company; ?> <?php if(!empty($sg_acct_num)) echo "(acct num: $sg_acct_num; )" ?> has requested access to data for herd <?php echo $herd_code; ?> using your herd release code.  If you accept this request, you do not need to do anything.  If you do not want to allow access,  <?php echo anchor('auth/service_grp_access/'. $id, 'Click this link');?> and revoke access.  If that link does not work, please cut and paste the following URL into your browser: <?php echo $this->config->item('base_url') . $this->config->item('index_page') . 'auth/service_grp_access/'. $id; ?></p>
