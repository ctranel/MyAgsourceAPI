...
class Data_entry_model extends CI_Model implements iForm_Model {
    ...
    public function getFormsByPage($page_id) {
        $page_id = (int)$page_id;
        $results = $this->db
            ->select('pb.page_id, b.id, b.name, f.id AS form_id, f.name AS form_name, b.description, dt.name AS display_type, s.name AS scope, b.isactive, b.path, f.dom_id, f.action, f.initial_defaults_url, pb.list_order')
...
//            ->where('')
    ...
    public function getFormByBlock($block_id) {
        ...
        $results = $this->db
            ->select('b.id, b.name, f.id AS form_id, f.name AS form_name, b.description, dt.name AS display_type, s.name AS scope, b.isactive, b.path, f.dom_id, f.action, f.initial_defaults_url, 1 AS list_order') //, pb.list_order, pb.page_id,
...
//            ->where('')
    ...
    public function getFormById($form_id) {
        ...
        $results = $this->db
            ->select('b.id, b.name, f.id AS form_id, f.name AS form_name, b.description, dt.name AS display_type, s.name AS scope, b.isactive, b.path, f.dom_id, f.action, f.initial_defaults_url')
