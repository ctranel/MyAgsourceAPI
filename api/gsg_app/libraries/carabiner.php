...
class Carabiner {
	...
	*/	
	...
	{	
		...
		if(strpos(FCPATH,'\\') !== FALSE){ //windows path
			$this->script_path = str_replace('/', '\\', FCPATH.$this->script_dir);
			$this->style_path = str_replace('/', '\\', FCPATH.$this->style_dir);
			$this->cache_path = str_replace('/', '\\', FCPATH.$this->cache_dir);			
		}
		else {		
	...
	*/
	private function _combine($flag, $files, $filename)
	{
		...
		foreach($files as $file):
			...
			if( (isset($file['minify']) && $file['minify']) || (!isset($file['minify']) && $minify) ):
	...
	private function _minify($flag, $file_ref)
	{
		...
		switch($flag){
			...
			case 'css':
				...
				$path = NULL;
				if($this->isURL($ref)){
					$path = substr($file_ref, 0, (strrpos($file_ref, '/') + 1));
				}
				$contents = $this->_get_contents( $ref );
				return $this->CI->cssmin->minify($contents, FALSE, $path);
